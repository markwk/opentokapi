(function ($) {
  ...
  Drupal.openTok.registerConnection = function(e){
    $.ajax({
      type: 'POST',
      url: Drupal.settings.basePath + 'opentokapi/save/connection',
      data: ({opentokapi_connection: JSON.stringify(e.openTok.eventData.target.connection)})
    });
  ...
  return $.ajax({
      type: 'POST',
      url: Drupal.settings.basePath + 'opentokapi/get/streamwrapper',
      data: ({opentokapi_stream: JSON.stringify(stream), opentokapi_session: JSON.stringify(session)}),
      async: false,
      dataType: 'html'
    }).responseText;
  };
